...
use App\Http\Controllers\{
    ...
    // Core flow (matches mobile 1:1)
    OrderController,
    PurchaseController,
    JobController,
    JobReceiveController,
    SaleController,
...
Route::middleware(['auth'])->group(function () {
    ...
    //
    // uri => [controller, permission_prefix]
    // Auto-generates: index/create/store/show/edit/update/destroy/print
    // ────────────────────────────────────────────────────────────────
    ...
    $modules = [
        ...
        // Core flow — matches mobile dashboard/bottom-nav exactly
        'orders'                 => [OrderController::class,              'orders'],
        'purchase'                => [PurchaseController::class,          'purchase'],
        'jobs'                   => [JobController::class,                'jobs'],
        'job_receives'           => [JobReceiveController::class,         'job_receives'],
    ...
    // ────────────────────────────────────────────────────────────────
    // JOB RECEIVE — nested action under a job (mirrors mobile's
    // "receive" icon on the job card, which opens Add Job Receive)
    // ────────────────────────────────────────────────────────────────
    Route::get('jobs/{job}/receive',  [JobReceiveController::class, 'createForJob'])
        ->middleware('check.permission:job_receives.create')
        ->name('jobs.receive.create');
    ...
    // ────────────────────────────────────────────────────────────────
    // REPORTS — matches mobile Reports hub + Party Ledger tile
    // ────────────────────────────────────────────────────────────────
